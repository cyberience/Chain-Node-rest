...
var idReducer = {
  ...
  */
	createIdToNameLookup: function createIdToNameLookup(nameTuples) {
		var nextRoundKeys = [];

		// copy all tuples into an id -> value dictionary
		var lookupMap = {};
		nameTuples.forEach(function (nameTuple) {
			var key = uint64.toHex(nameTuple.namespaceId);
		...
		var processRoundKeys = function processRoundKeys(roundKeys) {
			var additionalProcessingKeys = [];
			roundKeys.forEach(function (key) {
				var nameTuple = lookupMap[key];
				var parentEntry = lookupMap[uint64.toHex(nameTuple.nextId)];
		...
		while (0 !== nextRoundKeys.length) {
			var currentRoundKeys = nextRoundKeys.slice();
    ...
    */
			findName: function findName(id) {
				return (lookupMap[uint64.toHex(id)] || {}).fqn;
			}
		};
	}
};
...
//# sourceMappingURL=idReducer.js.map