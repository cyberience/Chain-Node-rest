...
var address = {
  ...
  */
	addressToString: function addressToString(decoded) {
		if (constants.sizes.addressDecoded !== decoded.length) throw Error(convert.uint8ToHex(decoded) + ' does not represent a valid decoded address');
  ...
  */
	publicKeyToAddress: function publicKeyToAddress(publicKey, networkIdentifier) {
		...
		var publicKeyHash = sha3_256.arrayBuffer(publicKey);

		// step 2: ripemd160 hash of (1)
		var ripemdHash = new Ripemd160().update(Buffer.from(publicKeyHash)).digest();
		...
		var hash = sha3_256.arrayBuffer(decodedAddress.subarray(0, constants.sizes.ripemd160 + 1));
